...
export default class ScummvmFS {
    url;
    fs_index;
    stream_ops;
    node_ops;
    ...
    read(args) {
        ...
        if (alreadyLoaded) {
            ...
            for (var block = first_block; block <= last_block; block++) {
                ...
                for (idx = Math.max(start, block * RANGE_REQUEST_BLOCK_SIZE); idx <= Math.min(end, block * RANGE_REQUEST_BLOCK_SIZE + this.fs_index[path].data[block].length - 1); idx++) {
...
console.log('download: ' + path);
                ...
                if (req.status === 200) { // range request wasn't respected or requested
                    ...
                    if (char_length === 2 && text.length === (range_end - range_start + 1) / 2 - 1) {
                        ...
                        for (let i = 0; i < (range_end - range_start + 1); i++) {
                            block = Math.floor((range_start + i) / RANGE_REQUEST_BLOCK_SIZE)
                            if (self.fs_index[path].data[block] === undefined) {
                                self.fs_index[path].data[block] = new Uint8Array(RANGE_REQUEST_BLOCK_SIZE)
                            }
                    ...
                    console.error(req);
                    throw new this.FS.ErrnoError(ERRNO_CODES.EINVAL);
                }
            }
        };
        req.send();
    ...
    convertResult(result) {
        if (result.ok) {
            ...
            if (result.status === 404) {
                error = new this.FS.ErrnoError(ERRNO_CODES.ENOENT);
            }
            else {
                error = new this.FS.ErrnoError(ERRNO_CODES.EPERM);
            }
    ...
    node_ops = {
        ...
        lookup: (parent, name) => {
            logger(name, "lookup ")
            if (parent instanceof this.FS.FSStream) { //sometimes we get a stream instead of a node
                parent = parent.node;
            }
    ...
    stream_ops = {
        ...
        close: (stream) => {
            ...
            const path = realPath(stream.node);
            if (this.FS.isFile(stream.node.mode) && stream.fileData) {
